...
class Brick {
    ...
    getRowPositionBrick() {
        ...
        for (let y = 0; y < this.layout[this.direction].length; y++) {
            // Duyet theo cot
            for (let x = 0; x < this.layout[this.direction][0].length; x++) {
                // Kiem tra xen hang do co chua mot gia tri khac background_id khong neu khac thi co nghia hang do chua gia tri cua brick
                if (this.layout[this.direction][y][x] !== background_id) {
...
document.getElementById('playAudio').addEventListener('click', () => {
    // Kiem tra game co dang coi hay khong
    if (board.play) {
        // Keim tra xem game dang over hay chua
        if (!board.over) {
            document.getElementById('demo').play();
            // Cap nhat lai = true de cho phep mo nhac
...
document.getElementById('pauseAudio').addEventListener('click', () => {
    ...
    if (board.play) {
        ...
        if (!board.over) {
            document.getElementById('demo').pause();
            // Cap nhat lai = false de khong cho phep mo nhac
